...
#每次更新前，先重置更改过的文件
git checkout -- /root/.tlgame/gs/services/server/config/odbc.ini
git checkout -- /root/.tlgame/gs/scripts/LoginInfo.ini
git checkout -- /root/.tlgame/gs/scripts/ShareMemInfo.ini
git checkout -- /root/.tlgame/gs/scripts/config.json
git checkout -- /root/.tlgame/gs/scripts/ServerInfo.ini
#
export $(grep -v '^#' /root/.tlgame/gs/.env | xargs -d '\n')
